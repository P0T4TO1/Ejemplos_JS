...
const pokedex = () => {
  ...
  const imageTemplate =
    "<img class='pokedisplay' src='{imgSrc}' alt='pokedisplay'/>";
  ...
  const checkDisabled = (button) => {
    button.disabled =
      button.id === "btnDown" && containers.pokemonIdElement.value <= 1;
  };
  ...
  const triggers = () => {
    ...
    buttons.next.onclick = () =>
      setPokemonData(+containers.pokemonIdElement.value + 1);
